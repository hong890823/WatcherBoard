package com.hong.watcherboard.view;
...
import com.hong.watcherboard.R;
import com.hong.watcherboard.util.SizeUtil;
import com.hong.watcherboard.util.klog.KLog;
 ...
 * 参考博客：http://blog.csdn.net/qq_26971803/article/details/52061943
 */

public class WatcherBoard extends View {
    private static final String TAG = "WatcherBoard";
    ...
    //onSizeChanged方法夹杂在多次执行的onMeasure方法中，只执行一次
    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        Log.d(TAG, "onSizeChanged: ");
        super.onSizeChanged(w, h, oldw, oldh);
        mRadius = (Math.min(w, h) - getPaddingLeft() - getPaddingRight()) / 2;
        mPointEndLength = mRadius / 6; //尾部指针默认为半径的六分之一，包括时针，分针，秒针
    }
    ...
    protected void onDraw(Canvas canvas) {
        Log.d(TAG, "onDraw: ");
        canvas.save();
        canvas.translate(getWidth() / 2, getHeight() / 2);//确定表盘中心,把坐标原点放在View的中心上
        //绘制外圆背景
        paintCircle(canvas);
        //绘制刻度
        paintScale(canvas);
    ...
    private void paintPointer(Canvas canvas) {
        ...
        int angleHour = (hour % 12) * 360 / 12; //时针转过的角度 实际上应该是(hour % 12) * （360 / 12），这是什么率，忘了......
        int angleMinute = minute * 360 / 60; //分针转过的角度
        int angleSecond = second * 360 / 60; //秒针转过的角度
        //绘制时针
        canvas.save();
        canvas.rotate(angleHour+angleMinute/12); //旋转到时针的角度。12是因为分针转360时针转30，不需要考虑临界点问题因为不存在60分的时候，59分之后就是00分
        ...
        //绘制分针
        canvas.save();
        canvas.rotate(angleMinute+angleSecond/60);//60是因为秒针转360分针转6
    ...
    private void paintScale(Canvas canvas) {
        ...
        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                ...
                canvas.translate(0, -mRadius + DptoPx(5) + lineWidth + mPadding + (textBound.bottom - textBound.top) / 2);//在Y轴上移动坐标原点，y坐标是从-mRadius逐步减值的
                mPaint.setStyle(Paint.Style.FILL);
                canvas.rotate(-6 * i);//保证文字都是水平的
            ...
            canvas.rotate(6);//每个循环让坐标系旋转6度，60个循环之后就是360度，表盘的指针就画出来了
        }
    }
